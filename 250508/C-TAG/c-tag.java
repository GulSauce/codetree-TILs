...
class Solver {
    ...
    public void solve() {
        ...
        int answer = 0;
        int stringLength = A.get(0).length();
            ...
            for (int j = i + 1; j <= stringIndex; j++) {
                for (int k = j + 1; k <= stringIndex; k++) {
                    for (int cur = 0; cur <= groupElementIndex; cur++) {
                        ...
                        String sub = new String(new char[]{curString.charAt(i), curString.charAt(j), curString.charAt(k)});
                        aGroupExistString.get(i).get(j).get(k).add(sub);
                    }
                }
            }
        }
            ...
            for (int j = i + 1; j <= stringIndex; j++) {
                for (int k = j + 1; k <= stringIndex; k++) {
                    for (int cur = 0; cur <= groupElementIndex; cur++) {
                        ...
                        String sub = new String(new char[]{curString.charAt(i), curString.charAt(j), curString.charAt(k)});
                        bGroupExistString.get(i).get(j).get(k).add(sub);
                    }
                }
            }
        }
